...
### Fixed — 图片预览与文件识别
- 支持带 MPO 附加帧的手机 JPEG，以主照片生成预览；高分辨率 JPEG 在解码器缩小后检查像素上限，修复可正常上传却返回 415 的情况。
- Drop 预览失败时显示具体原因和「重试预览」，服务端记录失败原因，不再静默退回普通文件气泡。
- Drop 按文件内容识别浏览器未提供正确 MIME 的 JPEG 等图片；历史记录中的普通文件类型图片也可生成缩略图、点击查看原图，无需重新上传。
