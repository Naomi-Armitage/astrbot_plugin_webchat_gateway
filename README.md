...
`redis>=4.2` 已是必需依赖（写在 `requirements.txt` 顶层）。即使 `streaming.redis_dsn` 留空（走内存缓冲），导入仍然会发生；缺包则插件加载失败。如果确实不要 Redis 也想最小化依赖，可以删 `requirements.txt` 里那一行 —— `RedisBuffer` 是 lazy import，没安装包时单进程内存缓冲一样能用：
...
用户点登出按钮时，前端用 `navigator.sendBeacon` 调它（fallback `fetch` keepalive）。服务端记录 token 名 + 当前时间戳到内存表，从此刻起所有签发时间早于该时间的 cookie 一律拒绝。返回 `Set-Cookie: wcg_file=; Max-Age=0` 把浏览器侧 cookie 也清掉。端点本身受 IP 防爆破保护：未授权请求（无 bearer + 伪造 cookie）会累计 IP 失败计数，触发 `429 ip_blocked` 后关闭探测窗口。

### 其他用户端点（v0.3.0+）

下面这组面向「带状态聊天页」/「多设备同步」/「流式 + SSE 恢复」/「单条消息操作」/「会话标题」/「事件长轮询」六个域。所有端点 bearer 鉴权与 `/chat` 一致；CORS / Origin 检查同样走 `gate_request`。

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `{prefix}/chat/stream` | SSE 流式回复：响应是 `text/event-stream`；首帧 `{stream_id}` 用于 resume / cancel。每帧 `data: {chunk, seq}`，终帧 `{done:true, seq, remaining, daily_quota, incomplete?}` 或 `{error, seq}`。同样的 422/429/500/502/504 错误码体系，但 502/504 是 `data: {error:...}` 帧而不是 HTTP 状态。 |
| GET | `{prefix}/chat/stream/{stream_id}/resume?after_seq=N` | 断线重连：续传该 stream `seq > N` 的所有 chunk。同 token 同 session_id；陌生 stream 返回 404（不区分跨 token / 不存在，避免枚举）。stream 关闭后保留 `streaming.grace_seconds`（默认 30s）供 resume。 |
| POST | `{prefix}/chat/stream/{stream_id}/cancel` | 用户主动中断流式回复。**会真正终止服务端 LLM 调用**（不只是断开 SSE）。响应 204；陌生 stream 同样 404。 |
| GET | `{prefix}/me` | 身份探测 + `wcg_file` cookie 下发。返回 `{name, remaining, daily_quota}`。前端聊天页 boot 时调一次，浏览器拿到 HttpOnly cookie 后续 `<img>` 才能 GET 私有附件。 |
| POST | `{prefix}/title` | `{session_id, conversation:[{role,text}, ...]}` → `{title}`。可选服务端：LLM 总结前几轮生成 25 字以内会话标题，自动写回 session_meta。`auto_title_enabled=false` 时返回 503 disabled。 |
| GET | `{prefix}/conversations` | 列出本 token 名下所有会话：`{conversations:[{session_id, title, updated_at, message_count, preview, pinned, title_manual}], last_pts}`。`last_pts` 是当前事件流游标，用于后续长轮询。 |
| GET | `{prefix}/conversations/{sid}` | 单会话详情：`{session_id, title, ..., messages:[{role, content, ts, attachments?, incomplete?}]}`。复用 CM 历史 + `webchat_updates` 事件回放补齐附件与未完整标记。 |
| PATCH | `{prefix}/conversations/{sid}` | 改 `{title?, title_manual?, pinned?, deleted?}`（软删）。返回更新后的 `_meta_payload`。 |
| POST | `{prefix}/conversations/{sid}/clear` | 清空本会话历史 + 释放所有附件。返回 `_meta_payload`。与 `/chat/stream` 互斥，冲突 `429 concurrent_request`。 |
...
| PATCH | `/api/webchat/admin/tokens/{name}` | 改 `daily_quota` / `note` |
| POST | `/api/webchat/admin/tokens/{name}/regenerate` | 旋转该 token 的 hash：旧 bearer 立即失效（含已签发的 `wcg_file` cookie，HMAC 折入了 token_hash）。返回新 token。 |
...
管理面板（Web UI）登录走 cookie-session 而不是 bearer，三个配套端点：

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| POST | `/api/webchat/admin/login` | body `{key}`：验证 `master_admin_key` 后下发 HttpOnly 管理 cookie。 |
| POST | `/api/webchat/admin/logout` | 服务端记录 cookie 失效；同步清浏览器 cookie。 |
| GET | `/api/webchat/admin/me` | 探测当前 cookie 是否有效。 |
...
- **登出 cookie 服务端失效是单进程的**（v0.3.0+）：`POST {prefix}/files/logout` 在进程内的 `CookieLogoutTracker` 里记下时间戳，验证 cookie 时签发时间早于该值的一律拒绝。**多 worker 部署**下，每个 worker 独立持有 tracker，logout 只在收到该请求的那个 worker 立刻生效；其他 worker 继续认旧 cookie 直到自然过期（cookie TTL 写死 24h，见 `core/file_cookie.py:DEFAULT_TTL_SECONDS`）或下次插件重启（`file_cookie_secret` 轮转一次性失效所有 cookie）。缓解：(a) 反代上启用 sticky session 让同一用户固定到同一 worker；(b) 真正需要硬踢的场景用 admin `regenerate_token` —— 它轮转 `token_hash`，所有 worker 验证 cookie 时都会立刻拒绝。
...
- **MySQL 连接池**：`storage.mysql_pool_max`（默认 5、范围 1-100）可调；朋友规模 5 够用，5+ 并发或暴露给小团队建议 20-50。
- **per-message 操作是真删除**（v0.3.0+）：聊天页的"删除"按钮（`DELETE /messages/{idx}`）和"重新生成"按钮（`POST /regenerate`）都直接动 CM 历史，无 undo。删除会释放该条独享的附件。
- **regenerate 是 truncate 而非 in-place 替换**（v0.3.0+）：对中间的 bot 回复点"重新生成"会把它**之后所有 turn**（包括用户后续追问 + 对应回复）一并删掉，再追加新 assistant。对端设备会通过 `message_deleted` 事件流逐条同步看到删除。如果用户预期"只换这一条回复保留后面对话"，目前不支持 —— 受限于 LLM 调用本身需要正确的 context window。
- **`/chat/stream/{stream_id}/cancel` 会真正终止 LLM 调用**（v0.3.0+）：不只是断 SSE。如果你在反代日志里看到 "stream ended early but LLM still running" 一类问题，先确认不是用户主动 cancel —— audit `chat_stream_cancelled` 是确诊信号。
