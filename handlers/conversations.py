...
EVENT_MESSAGE_FOLDED = "message_folded"
EVENT_MESSAGE_INSERTED = "message_inserted"
...
def _renderable_entry(item: Any) -> tuple[str, str, list[str], bool] | None:
    """Return `(role, text, file_ids, folded)` if the CM entry should render, else None.
    ...
    `folded` mirrors the entry's `folded` field (defaulting to False).
    A folded entry is kept in the rendered list — the client renders
    it inside the fold-bar UI rather than as a normal bubble — so the
    rendered-index ↔ raw-index mapping stays stable as users edit /
    regenerate (the alternative, dropping folded entries here, would
    shift every downstream index whenever a fold flag flipped).
    """
    ...
    return role, text, file_ids, folded
...
def _normalize_history(raw: Any) -> list[dict]:
    ...
    for item in raw:
        ...
        if kept is None:
            continue
        role, text, file_ids, folded = kept
...
class ConversationService:
    ...
    async def edit_user_message(
        ...
        *,
        ...
        """Non-destructive edit of a user message.

        Marks the target user message and (if present) the assistant
        reply that immediately follows it as `folded`, then runs the
        LLM against the conversation context PRECEDING the edit point
        plus the new user text. The new user message + LLM reply are
        spliced into the raw history right after the folded pair, so
        any downstream history the user accumulated stays in place
        and continues to participate in future LLM context.

        Same concurrency / quota / audit envelope as
        `regenerate_assistant_message`. Returns the same
        `RegenerateResult` shape because the HTTP surface is symmetric
        with regenerate.
        """
        if self._llm_bridge is None:
            raise ServiceError("internal_error", status=500)
        ...
        if not text:
            raise ServiceError("invalid_payload", status=400)
        # Cap edit length at the same ~8k bound the streaming chat path
        # uses for the message field. Mismatch here would let a user
        # bypass the chat-time length check via the edit endpoint.
        if len(text) > 8192:
            raise ServiceError("payload_too_large", status=413)
        async with self._with_concurrency(
            token_name=token_name,
            operation="edit_user_message",
            session_id=session_id,
            ip=ip,
        ):
            return await self._edit_user_inner(
                token_name=token_name,
                session_id=session_id,
                message_index=message_index,
                new_text=text,
    ...
    async def regenerate_user_message(
        self,
        *,
        token_name: str,
        session_id: str,
        ...
        """Non-destructive regenerate from a user message.

        Same shape as `edit_user_message` minus the text change. The
        target user message stays live; the assistant reply that
        immediately follows it (if any) is folded; a new LLM reply is
        spliced in right after the folded entry. Downstream history is
        preserved.

        Bot-side `regenerate_assistant_message` is destructive (drops
        the tail) — this user-side variant is the non-destructive twin
        the chat UI offers on user-message hover.
        """
        if self._llm_bridge is None:
            raise ServiceError("internal_error", status=500)
        async with self._with_concurrency(
            token_name=token_name,
            operation="regenerate_user_message",
            session_id=session_id,
            ip=ip,
        ):
            return await self._regenerate_user_inner(
                token_name=token_name,
                session_id=session_id,
                message_index=message_index,
                username=username,
    ...
    @staticmethod
    def _next_renderable_raw_idx(raw: list[Any], start: int) -> int | None:
        """Find the next raw index whose entry is renderable (user/assistant
        with text or attachments). Returns None if none exists.

        Used by edit/regen-user to locate the assistant reply that
        immediately follows a user message in the rendered list — that
        reply (if any) is what gets folded alongside an edit and is
        what regen-user replaces with a new reply. Non-renderable
        entries (system, tool, etc.) are skipped because they don't
        appear in the chat UI and shouldn't influence the fold target.
        """
        for j in range(start, len(raw)):
            if _renderable_entry(raw[j]) is not None:
                return j
        return None

    @staticmethod
    def _filter_llm_context(raw: list[Any], upto_raw_idx: int) -> list[Any]:
        """Return raw[:upto_raw_idx] with folded entries dropped.

        Used to temporarily install a "what the LLM should see" history
        in CM before calling generate_reply. `_history_text` (and the
        provider's prompt builder) reads from CM directly, so the only
        way to hide folded entries from the LLM is to write a filtered
        slice in before the call. The full history (folded entries +
        new entries + downstream) is written back afterwards.
        """
        return [item for item in raw[:upto_raw_idx] if not (isinstance(item, dict) and item.get("folded"))]

    def _rendered_index_for_raw(self, raw: list[Any], raw_idx: int) -> int:
        """Count renderable entries in raw[:raw_idx]. Equals the rendered
        index of an entry that lives at raw[raw_idx] (if renderable),
        or the rendered position the next entry would take after an
        insert at raw_idx. Used to compute payload indices for the
        `message_folded` / `message_inserted` events we emit so peers
        can splice locally without re-fetching."""
        n = 0
        for j in range(min(raw_idx, len(raw))):
            if _renderable_entry(raw[j]) is not None:
                n += 1
        return n

    async def _edit_user_inner(
        self,
        *,
        token_name: str,
        session_id: str,
        ...
        raw_history, cid, target_raw_idx, target_entry, target_role = (
            await self._resolve_target_raw_index(
                token_name=token_name,
                session_id=session_id,
                message_index=message_index,
        ...
        if target_role != "user":
            raise ServiceError("message_not_found", status=404)
        if target_entry.get("folded"):
            # Editing a folded entry has no clean meaning — the user
            # only sees a fold-bar at that slot. Refuse rather than
            # silently re-folding noise.
            raise ServiceError("message_not_found", status=404)

        # Locate the assistant reply that immediately follows the user
        # message in rendered order. `target_raw_idx + 1` might be a
        # tool/system entry that the UI doesn't show, so walk forward
        # to the next renderable position.
        next_raw_idx = self._next_renderable_raw_idx(
            raw_history, target_raw_idx + 1
        )
        old_bot_raw_idx: int | None = None
        if next_raw_idx is not None:
            nxt = raw_history[next_raw_idx]
            if isinstance(nxt, dict) and str(nxt.get("role") or "").strip().lower() == "assistant":
                old_bot_raw_idx = next_raw_idx

        # Quota check BEFORE any side effect (matches /chat + regenerate).
        today = date.today()
        try:
            today_count = await self._storage.get_today_usage(
                token_name, day=today
            ...
            logger.exception(
                "[WebChatGateway] get_today_usage during edit failed"
            )
            today_count = 0
        if today_count >= token_daily_quota:
            await self._audit.write(
        ...
        # Compose the "marked" history: a copy with fold flags applied
        # but NO inserts yet. We'll temp-write a filtered slice for the
        # LLM, then write the marked+inserted version after the call.
        marked_history: list[Any] = list(raw_history)
        marked_history[target_raw_idx] = {
            **raw_history[target_raw_idx], "folded": True
        }
        if old_bot_raw_idx is not None:
            marked_history[old_bot_raw_idx] = {
                **raw_history[old_bot_raw_idx], "folded": True
            }

        # LLM context = visible (non-folded) raw entries before the
        # edit point + the new user text wrapped in a UserMessageSegment
        # so the prompt builder sees the same shape as live chats.
        llm_context = self._filter_llm_context(raw_history, target_raw_idx)
        new_user_entry = UserMessageSegment(
            content=[TextPart(text=new_text)]
        ).model_dump()
        llm_context.append(new_user_entry)
        ...
        try:
            ...
            logger.exception(
                "[WebChatGateway] CM.update_conversation(edit truncate) failed"
            )
            raise ServiceError("edit_failed", status=500) from None
        ...
        try:
            ...
            if status == 500:
                logger.exception(
                    "[WebChatGateway] edit LLM call failed"
                )
            detail: dict[str, Any] = {"operation": "edit_user_message"}
            if status == 500:
                detail["error"] = str(exc)[:200]
            await self._audit.write(
                audit_event,
            ...
            # CM is currently holding the temp filtered context. Restore
            # the original raw history so a failed edit leaves no stale
            # state behind (matches `_regenerate_inner`'s implicit
            # behavior — its temp write IS the truncate, so there's
            # nothing to roll back; here we rolled forward optimistically
            # so we have to undo).
            try:
                await self._cm.update_conversation(
                    unified_msg_origin=umo,
                ...
                logger.exception(
                    "[WebChatGateway] CM.update_conversation(edit rollback) failed"
                )
            raise ServiceError(code, status=status) from None

        # Compose final history. Insert position = right after whatever
        # we folded (the original pair, or just the user if no bot
        # followed). marked_history[:insert_pos] preserves any non-
        # renderable system/tool entries that lived between the user
        # and the assistant reply, so we don't lose tool-call context.
        new_assistant_entry = AssistantMessageSegment(
            content=[TextPart(text=reply)]
        ).model_dump()
        if old_bot_raw_idx is not None:
            insert_pos = old_bot_raw_idx + 1
        else:
            insert_pos = target_raw_idx + 1
        final_history = (
            list(marked_history[:insert_pos])
            + [new_user_entry, new_assistant_entry]
            + list(marked_history[insert_pos:])
        )
        try:
            await self._cm.update_conversation(
            ...
            logger.exception(
                "[WebChatGateway] CM.update_conversation(edit write) failed"
            )
            raise ServiceError("edit_failed", status=500) from None

        # Quota increment AFTER persistence (matches regenerate ordering).
        try:
            new_count = await self._storage.increment_daily_usage(
                token_name, day=today
            )
            ...
            logger.exception(
                "[WebChatGateway] increment_daily_usage during edit failed"
            )
            new_count = today_count + 1
        remaining = max(0, token_daily_quota - new_count)
        ...
        new_rendered = _normalize_history(final_history)
        new_meta_count = len(new_rendered)
        # Preview uses the freshest visible bot reply — fall back to
        # the new reply itself if the visible-tail walk somehow misses.
        preview_text = reply[:_PREVIEW_CHARS]
        now = self._now()
        await self._storage.upsert_session_meta(
            token_name=token_name,
        ...
        # Emit events. Order matters: fold first (so peers mark old
        # entries collapsed BEFORE the new entries shift indices), then
        # insert ascending so each insert's index is valid against the
        # peer's already-mutated history at the moment of application.
        folded_user_render_idx = message_index
        folded_bot_render_idx = (
            message_index + 1 if old_bot_raw_idx is not None else None
        )
        events: list[NewEvent] = []
        events.append(
            NewEvent(
                event_type=EVENT_MESSAGE_FOLDED,
                session_id=session_id,
                payload=json.dumps(
                    {"index": folded_user_render_idx, "role": "user"},
                    ensure_ascii=False,
                ),
            )
        )
        if folded_bot_render_idx is not None:
            events.append(
                NewEvent(
                    event_type=EVENT_MESSAGE_FOLDED,
                    session_id=session_id,
                    payload=json.dumps(
                        {"index": folded_bot_render_idx, "role": "assistant"},
                        ensure_ascii=False,
                    ),
                )
            )
        # New user msg goes right after the folded pair (= +1 if no
        # bot was folded, +2 if it was). New bot reply is right after.
        insert_user_render_idx = (
            message_index + 2 if old_bot_raw_idx is not None else message_index + 1
        )
        events.append(
            NewEvent(
                event_type=EVENT_MESSAGE_INSERTED,
                session_id=session_id,
                payload=json.dumps(
                    {
                        "index": insert_user_render_idx,
                        "role": "user",
                        "content": new_text,
                    },
                    ensure_ascii=False,
        ...
        events.append(
            NewEvent(
                ...
                payload=json.dumps(
                    {
                        "index": insert_user_render_idx + 1,
                        "role": "assistant",
                        "content": reply,
                    },
                    ensure_ascii=False,
    ...
    async def _regenerate_user_inner(
        self,
        *,
        token_name: str,
        session_id: str,
        ...
        # Recover text + image refs from the live user entry (we're
        # regenerating its reply, not the user message itself).
        user_text = _extract_text(target_entry.get("content"))
        user_file_ids = list(
            _extract_attachment_file_ids(target_entry.get("content"))
        )
        if not user_text and not user_file_ids:
            # Empty user turn with no attachments — nothing meaningful
            # to feed the LLM. Reject rather than send an empty prompt.
            raise ServiceError("message_not_found", status=404)
        ...
        try:
            ...
            logger.exception(
                "[WebChatGateway] get_today_usage during regenerate_user failed"
            )
            today_count = 0
        if today_count >= token_daily_quota:
            await self._audit.write(
        ...
        # Resolve image attachments to provider-visible paths so multi-
        # modal user turns regenerate multimodally (same as bot regen).
        image_urls: list[str] = []
        for fid in user_file_ids:
            try:
                ...
                logger.exception(
                    "[WebChatGateway] get_file during regenerate_user failed"
                    " file_id=%s",
                    fid,
                )
                continue
            ...
            if row.token_name != token_name or row.session_id != session_id:
                logger.warning(
                    "[WebChatGateway] regenerate_user skipped cross-scope"
                    " attachment token=%s session=%s file=%s",
                    token_name,
                    session_id,
                    fid,
            ...
            try:
                ...
                logger.exception(
                    "[WebChatGateway] open_local_path during regenerate_user"
                    " failed key=%s",
                    row.storage_key,
                )
                continue
        ...
        # LLM context includes the live user msg (target stays unfolded)
        # but skips any folded entries from prior edits.
        llm_context = self._filter_llm_context(
            raw_history, target_raw_idx + 1
        )
        umo = _umo(token_name, session_id)
        try:
            ...
            logger.exception(
                "[WebChatGateway] CM.update_conversation(regenerate_user"
                " truncate) failed"
            )
            raise ServiceError("regenerate_failed", status=500) from None
        ...
        try:
            ...
            if status == 500:
                logger.exception(
                    "[WebChatGateway] regenerate_user LLM call failed"
                )
            detail: dict[str, Any] = {"operation": "regenerate_user_message"}
            if status == 500:
                detail["error"] = str(exc)[:200]
            await self._audit.write(
                audit_event,
        ...
        try:
            ...
            logger.exception(
                "[WebChatGateway] increment_daily_usage during regenerate_user"
                " failed"
            )
            new_count = today_count + 1
        remaining = max(0, token_daily_quota - new_count)
...
def make_conversation_handlers(
    ...
    async def edit_message(request: web.Request) -> web.Response:
        """POST {prefix}/conversations/{session_id}/messages/{message_index}/edit

        Body: `{"new_text": str}`. Non-destructive edit of a user
        message — folds the old user msg + adjacent bot reply (if
        any), splices in a new user msg + LLM reply, leaves downstream
        history intact. Same response shape + error codes as
        /regenerate.
        """
        gated = await gate_request(request, deps)
        if isinstance(gated, web.Response):
        ...
        try:
            ...
            logger.exception("[WebChatGateway] edit_message parse failed")
            return _err(
                request, gated.origin, ServiceError("invalid_json", status=400)
            )
        if not isinstance(body, dict):
        ...
        try:
            ...
            logger.exception("[WebChatGateway] edit_message failed")
            return _err(
                request, gated.origin, ServiceError("internal_error", status=500)
            )
        return json_response(
    ...
    async def regenerate_user(request: web.Request) -> web.Response:
        """POST {prefix}/conversations/{session_id}/messages/{message_index}/regenerate

        Non-destructive regenerate based on a user message — the user
        message stays live, the assistant reply that followed it (if
        any) is folded, a new LLM reply is spliced in, downstream
        history stays. Distinct from the bot-side
        /conversations/{sid}/regenerate which is destructive and
        truncates the tail.
        """
        gated = await gate_request(request, deps)
        if isinstance(gated, web.Response):
        ...
        try:
            result = await service.regenerate_user_message(
                token_name=gated.token.name,
                session_id=session_id,
                message_index=message_index,
                token_daily_quota=gated.token.daily_quota,
            ...
            logger.exception("[WebChatGateway] regenerate_user failed")
            return _err(
                request, gated.origin, ServiceError("internal_error", status=500)
            )
        return json_response(
    ...
    return {
        ...
        "regenerate_user": regenerate_user,
        "events": get_events,
        "preflight": preflight,
    }
